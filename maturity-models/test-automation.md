...
- □ AIによるテストギャップ分析や自動提案を活用している

### Component/Integration Tests（コンポーネント/統合テスト）

#### 評価軸1: 網羅性（Coverage）

**評価対象**: 内部統合ユースケース（機能単位） - 複数の内部コンポーネント/モジュールが連携して実現する機能
...
**レベル5: 最適化段階**
- □ 開発者主導のテスト文化が組織全体に定着している
- □ テストなしのDev Completeが組織文化として許容されない
...
- □ ビジネスクリティカルな機能のみテスト合格がDev Completeの条件

**レベル3: 定義段階**
- □ テスト戦略が詳細に定義されている（内部統合ポイント一覧、各ポイントのテスト方針、異常系カバレッジ等）
- □ 統合テスト合格がDev Completeの前提条件として標準化されている

**レベル4: 定量管理段階**
- □ 契約テストの戦略が文書化されている（プロバイダー・コンシューマー双方のテスト方針）
- □ すべての統合テスト + 契約テスト合格がDev Completeの必須条件
...
**評価対象**: 外部連携ユースケース（連携単位） - 外部システムとの1つの連携操作（読み書き、API呼び出し等）
...
- □ 外部システム統合テスト合格がDev Completeの前提条件として標準化されている

**レベル4: 定量管理段階**
- □ 障害シナリオをカバーするテスト戦略が文書化されている
- □ すべての外部システムテスト + 障害シナリオテスト合格がDev Completeの必須条件
...
**評価対象**: エンドツーエンドユースケース（ユーザージャーニー） - ユーザーがシステムを使って達成する一連の業務フロー
...
- □ E2Eテスト合格がDev Completeの前提条件として標準化されている

**レベル4: 定量管理段階**
- □ クロスブラウザテスト戦略が文書化されている
- □ すべてのE2Eテスト + クロスブラウザテスト合格がDev Completeの必須条件
...
**最終更新**: 2025-11-18
**バージョン**: 16.0 (全テストタイプにリスクベース戦略を拡張: レベル5の最適化を充実化)
