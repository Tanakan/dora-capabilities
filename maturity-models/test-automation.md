...
- □ 実行時間が基準を超えた場合、改善タスクが作成される
- □ Flaky Test率が5%未満（※ E2Eはブラウザ・ネットワーク依存のため、Unit/ITより基準を緩める）
- □ Flaky Testを検知したら自動でIssueが作成される
...
**レベル2: 管理段階**
- □ 一部の開発者がテストを書いている
- □ ATDD（Acceptance Test Driven Development）を試験的に導入している

**レベル3: 定義段階**
- □ 開発者がテストを作成・保守している（DORA原則）
- □ ATDDを実践している（受入基準を事前定義し、Given-When-Thenでシナリオを記述）

**レベル4: 定量管理段階**
- □ 開発者主導のテスト文化が定着している
- □ ATDDが推奨プラクティスとして定着している

**レベル5: 最適化段階**
- □ 開発者主導のテスト文化が組織全体に定着している
- □ ATDDが組織標準として定着し、ステークホルダー（PO、開発者、QA）が協働で受入基準を策定している
- □ テストなしの完了の定義（DoD）が組織文化として許容されない
...
**最終更新**: 2025-11-18
**バージョン**: 22.0 (軽微な問題4件を修正: E2E Flaky Test率の理由明記、External SystemsにFlaky Test率追加、CDC/Mutation Testing重複削除、ヒートマップ改善ガイド削除)
